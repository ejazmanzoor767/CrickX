...
import { approveContestPool, connectWallet, CRX_CONTEST_POOL_ADDRESS, joinOnchainContest, readCrxWallet } from '../../lib/web3';
...
function ContestContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Keep the initial contest render free of MetaMask and Polygon RPC work.
        // Those reads are only needed when the user actually joins.
        const [contestResult, teamsResult] = await Promise.all([
  ...
  async function join() {
    ...
    try {
      ...
      setContest((prev: any) => prev ? { ...prev, filledSpots: Number(prev.filledSpots || 0) + 1, prizePoolTotal: Number(prev.prizePoolTotal || 0) + Number(prepared.entryFee) } : prev);
  ...
  // The fast endpoint intentionally returns no chain object. The off-chain
  // contest is considered open until Join Contest performs the authoritative
  // Sportmonks + blockchain checks.
  ...
  return <section className="app-page" style={{ maxWidth: 980, paddingBottom: 96 }}>
    ...
    <div className="card" style={{ padding: 24, marginBottom: 14, background: 'linear-gradient(135deg,rgba(244,197,66,.10),rgba(18,23,34,.96))' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 12 }}>
        ...
        <div><span className="muted-label">PLAYERS</span><strong style={{ display: 'block', fontSize: 28, marginTop: 4 }}>{contest.filledSpots ?? 0}</strong><small className="section-subtitle">Unlimited</small></div>
        <div><span className="muted-label">POOL</span><strong style={{ display: 'block', fontSize: 28, marginTop: 4 }}>{contest.prizePoolTotal ?? 0} CRX</strong></div>
