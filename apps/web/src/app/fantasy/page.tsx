...
function FantasyContent() {
  ...
  async function loadMine(): Promise<FantasyTeamDto[]> {
    const [t, e] = await Promise.all([api.myFantasyTeams(), api.myEntries()]);
    const nextTeams = (unwrap(t) ?? []) as FantasyTeamDto[];
    setTeams(nextTeams);
    setEntries(unwrap(e) ?? []);
    return nextTeams;
  }

  async function loadMatchData(teamList: FantasyTeamDto[] = teams) {
    ...
    if (!editingTeamId) {
      const teamForFixture = (teamList as any[]).find((team) => Number(team.sportmonksFixtureId) === fixtureId && !team.isLocked);
      if (teamForFixture) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const loadedTeams = await loadMine();
        if (active) await loadMatchData(loadedTeams);
      } catch (err) {
  ...
  return (
    <div>
      ...
      {!hasFixture ? (
        <div className="card"><h2>Choose a match</h2><p className="section-subtitle">Open a match and choose Create XI to start building your fantasy team.</p><Link className="primary-button" href="/matches">Go to matches</Link></div>
      ) : (
        <>
          <div className="card"><div className="section-mini-row"><div><p className="eyebrow">TEAM STATUS</p><h2>{announced ? 'Playing XI confirmed' : 'Team setup'}</h2></div><span className={currentSavedTeam ? 'badge-live' : savedDraftForFixture ? 'badge-live' : 'demo-pill'}>{currentSavedTeam ? 'SAVED' : savedDraftForFixture ? 'PROGRESS SAVED' : 'READY'}</span></div><p className="section-subtitle">Your selections are kept with this match so you can leave and return without rebuilding your XI.</p></div>

          {teamsForFixture.map((team: any) => (
            <section className="match-section" key={team.id}>
              <div className="section-mini-row"><div><p className="eyebrow">{team.name}</p><h2>{team.playingXICount}/11 playing · {team.playerCount} squad</h2></div><span className="demo-pill">{selectedPlayers.filter((p) => p.team_id === team.id).length}/7 selected</span></div>
              <div className="match-list">
                {(team.players ?? []).map((player: any) => {
                  ...
                  return <article key={player.player_id} className="card" style={{ opacity: blocked ? .48 : 1 }}><div className="section-mini-row"><div><strong>{player.fullname ?? `Player ${player.player_id}`}</strong><div className="section-subtitle">{player.position_name ?? 'Player'} · {player.isPlayingXI ? 'PLAYING XI' : 'SQUAD'} · {Number(player.creditValue ?? DEFAULT_CREDITS).toFixed(1)} credits</div></div><button className={isSelected ? 'primary-button' : 'secondary-button'} onClick={() => togglePlayer(player)} disabled={blocked}>{isSelected ? '✓ Selected' : blocked ? 'Unavailable' : 'Select'}</button></div>{isSelected && <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:10 }}><button className={c ? 'primary-button' : 'secondary-button'} onClick={() => setCaptain(player.player_id)}>Captain</button><button className={vc ? 'primary-button' : 'secondary-button'} onClick={() => setViceCaptain(player.player_id)}>Vice-captain</button></div>}</article>;
                })}
              </div>
            </section>
          ))}

          <div className="card match-actions"><div><p className="eyebrow">{editingTeamId ? 'EDIT TEAM' : 'SAVE TEAM'}</p><h2>{editingTeamId ? 'Update your XI' : 'Save your XI'}</h2><p className="section-subtitle">{selected.length}/11 selected · Captain {captain ? 'set' : 'missing'} · Vice-captain {viceCaptain ? 'set' : 'missing'}</p><input value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="Fantasy team name" style={{ width:'100%', maxWidth:420, marginTop:10 }} />{message && <p className={message.includes('saved') || message.includes('updated') || message.includes('successfully') ? 'section-subtitle' : 'error-text'}>{message}</p>}</div><button className="primary-button" onClick={saveTeam} disabled={saving}>{saving ? 'Saving…' : editingTeamId ? 'Update Fantasy Team' : 'Save Fantasy Team'}</button></div>

          <div className="card"><div className="section-mini-row"><div><p className="eyebrow">CONTEST LOBBY</p><h2>Choose where to play</h2></div><span className="demo-pill">4 GEMS</span></div>{contests.length === 0 ? <p className="section-subtitle">No contest is available for this match yet.</p> : <><div style={{ display:'grid', gap:10 }}>{contests.map((c: any) => { const active = selectedContestId === c.id || (!selectedContestId && contests[0].id === c.id); return <button key={c.id} onClick={() => setSelectedContestId(c.id)} className={active ? 'primary-button' : 'secondary-button'} style={{ width:'100%', textAlign:'left' }}>{c.name} · 4 ◆ · {c.filledSpots}/{c.totalSpots} joined</button>; })}</div><div style={{ marginTop:12 }}><button className="primary-button" onClick={joinSelectedContest} disabled={joining || !selectedTeam}>{joining ? 'Joining…' : `Join Contest · ${ENTRY_FEE} ◆`}</button>{!selectedTeam && <p className="section-subtitle">Save the final XI first.</p>}</div></>}</div>
        </>
      )}
      ...
      <section className="match-section">
        ...
        {savedDraftForFixture && !currentSavedTeam && <div className="card"><div className="section-mini-row"><div><strong>{draft?.name || 'My CrickX XI'}</strong><div className="section-subtitle">{draft?.sportmonksPlayerIds?.length ?? 0}/11 players · Saved progress for this match</div></div><span className="badge-live">SAVED</span></div><div className="section-subtitle" style={{ marginTop: 8 }}>Your selections are stored and will reappear when you return to this match.</div></div>}
        {teams.length === 0 && !savedDraftForFixture ? <div className="card empty-state"><strong>No saved teams yet.</strong><span>Build a team above and save it here.</span></div> : teams.map((t: any) => <div key={t.id} className="card"><div className="section-mini-row"><div><strong>{t.name}</strong><div className="section-subtitle">{t.players.length} players · {t.isLocked ? 'Final' : 'Editable'}</div></div>{Number(t.sportmonksFixtureId) === fixtureId && !t.isLocked && <button className="secondary-button" onClick={() => editTeam(t)}>Edit team</button>}</div></div>)}
        <div style={{ marginTop: 28 }}><div className="section-mini-row"><div><p className="eyebrow">MY ENTRIES</p><h2>Contest history</h2></div><span className="demo-pill">RESULTS</span></div>{entries.length === 0 ? <div className="card empty-state"><strong>No contest entries yet.</strong><span>Joined contests will appear here with points and rank.</span></div> : entries.map((e) => <div key={e.id} className="card"><strong>{e.contest?.name ?? 'Contest'}</strong><div className="section-subtitle">Entry: {e.entryFeePaid?.toString?.() ?? 4} Gems · Points: {e.totalPoints ?? '—'} · Rank: {e.rank ?? '—'} · Prize: {e.prizeWon ?? '—'}</div></div>)}</div>
      </section>
    </div>
  );
}

export default function FantasyPage() { return <Suspense fallback={<div className="card skeleton-card">Loading your fantasy area…</div>}><FantasyContent /></Suspense>; }
